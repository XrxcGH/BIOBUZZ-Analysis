...
**[C] The R503 carve-outs, from the Table 12-1 notes (p. 75), matter for BOM planning:** two kinds of motor are exempt. The first is a vibration or autofocus motor factory-installed in a COTS computing device (a phone's rumble motor, for example), which may be used only inside that device and never removed or repurposed. The second is a motor built into a COTS sensor such as a LIDAR or scanning sonar, provided the sensor is unmodified apart from its mounting. For each, Table 12-1 states *"**These motors do not count toward the limit in R503.**"*
